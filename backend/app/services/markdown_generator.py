...
def generate_full_report_package(
    ...
    Args:
        ...
        template_data: Template classification results (optional)
        keyword_data: Keyword research results (optional)
    ...
    if template_data and template_data.get("templates"):
        reports["templates"] = _generate_template_report(site_url, template_data)

    # Keyword Research Report
    if keyword_data and keyword_data.get("keywords"):
        reports["keywords"] = _generate_keyword_report(site_url, keyword_data, plan_data.get("keyword_clusters", []))
...
def _generate_template_report(site_url: str, template_data: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"**Templates Identified:** {len(templates)}",
        "",
        "---",
        "",
        "## Template Overview",
    ...
    for template in templates:
        ...
        count = template.get("page_count", 0)
        desc = template.get("description", "")[:60]
        lines.append(f"| {name} | {count} | {desc}{'...' if len(template.get('description', '')) > 60 else ''} |")
    ...
    for template in templates:
        ...
        count = template.get("page_count", 0)
        examples = template.get("example_urls", [])
        recommendations = template.get("seo_recommendations", [])
    ...
    lines.extend([
        "---",
        "",
        "*Template analysis generated by SEOman*",
    ])

    return "\n".join(lines)


def _generate_keyword_report(
    site_url: str,
    ...
    sorted_keywords = sorted(keywords, key=lambda k: k.get("search_volume") or 0, reverse=True)
    ...
    lines = [
        ...
        f"**Total Search Volume:** {sum(k.get('search_volume') or 0 for k in keywords):,}",
        "",
        "---",
        "",
    ...
    for kw in sorted_keywords[:30]:
        ...
        difficulty = kw.get("difficulty") or "-"
        cpc = kw.get("cpc") or 0
        intent = kw.get("intent") or "-"
        lines.append(f"| {text} | {volume:,} | {difficulty} | ${cpc:.2f} | {intent} |")
